...
'"
  ...
  def issue_link_or_empty(item)
    ...
    item.new_record? ? "" : link_to(text, {:controller => "issues", :action => "show", :id => item}, {:target => "_blank", :class => "prevent_edit prevent_edit_exists_pic"})
  end
  ...
  def custom_fields_target_fixed_class(story)
    return '' if story.new_record?
    res = ''
    story.custom_field_values.each {|value|

      if value.custom_field.name == "対応範囲"
        cfn = show_value(value)
        if cfn == "画面"
          res = "icon-ui"
        elsif cfn == "サーバ"
          res = "icon-server"
  ...
  def custom_fields_target_fixed(story)
    return '' if story.new_record?
    res = ''
    story.custom_field_values.each {|value|
